...
def create_outliers_mask(sed: Table,
    ...
    def chi_sq_resids(y_model: np.ndarray, mask: np.ndarray) -> np.ndarray:
        return ((y[mask] - y_model) / (y_err[mask] + 1e-30))**2
    ...
    def objective_func(temps, mask) -> float:
        ...
        chi_sq_i = chi_sq_resids(scaled_bb_model(temps, mask), mask)
    ...
    for _iter in range(sed_count):
        ...
        chi_sq_i = chi_sq_resids(scaled_bb_model(soln.x, retain_mask), retain_mask)
